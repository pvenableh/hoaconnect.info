...
// Settings → Public site. Slimmed to infrastructure only: choose the built-in
// landing vs an external marketing site, and connect a custom/APEX domain. The
// actual landing DESIGN (sections, hero, theme, AI) now lives in the drag-and-
// drop builder at /admin/settings/site ("Design your site") — this page links to
// it. (The old monolithic editor was replaced by that builder.)
...
// ── External-site vs built-in landing ──────────────────────────────────────
...
const saveExternalUrl = async () => {
  ...
  try {
    ...
    toast.success(url ? "External site saved — built-in landing disabled" : "Built-in landing enabled");
    await load();
  } catch (e: any) {
...
const verify = async () => {
  ...
  try {
    ...
    if (res.verified) toast.success("Domain verified — your site is going live");
    else toast.warning(res.message || "Verification record not found yet");
    await load();
  } catch (e: any) {
...
<template>
  <div class="min-h-screen t-bg t-text">
    <PageContainer class="space-y-6">
      ...
      <template v-else>
        <!-- Design-your-site CTA -->
        <div class="ios-card p-6 flex flex-wrap items-center justify-between gap-4">
          ...
          <Button class="rounded-full" @click="navigateToOrg('/admin/settings/site')">
            <Icon name="lucide:paintbrush" class="w-4 h-4 mr-1.5" />
            Open builder
          </Button>
        </div>
        ...
        <div class="ios-card p-6 space-y-4">
          ...
          <p v-else class="text-xs t-text-muted">
            ...
            <button type="button" class="t-link" @click="navigateToOrg('/admin/settings/site')">Design it →</button>
          </p>
