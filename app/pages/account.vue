...
// A logged-in user's personal settings page is a WORKSPACE surface, so it wears
// `theme-app` like every other settings screen. It used to fall through to the
// public layout, which had a visible consequence: the Appearance card below
// hosts the light/dark toggle, and on a public theme the tokens are pinned light
// — so clicking "Dark Mode" flipped the label, saved the preference and added
// the `dark` class while the page in front of you did not change colour at all
// (measured: rgb(249,250,251) before and after). The control only appeared to
// work once you navigated somewhere else.
//
// `auth` middleware comes with it: the page reads `user.value` throughout and
// rendered an empty form for a signed-out visitor. Deliberately NOT
// `subscription` — someone whose subscription lapsed still needs to reach their
// own account.
definePageMeta({
  middleware: ["auth"],
  layout: "auth",
