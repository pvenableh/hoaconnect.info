          ...
          <div class="content-main max-w-4xl min-w-0 overflow-hidden">
            ...
            >
              ...
              <h3 class="t-heading text-2xl font-normal t-text mb-3">
                {{ signature.name }} — bespoke, from ${{
                  money(signature.fromUsd)
                }}
              </h3>
              <p
              ...
              <button
                ...
                class="t-btn px-6 py-3 text-sm font-medium transition-all duration-300 rounded-lg"
              >
                {{ signature.cta }}
              </button>
            </div>
...
// ============================================================================
// MARKETING CONTEXT (no auth/org here — CTAs link into the application;
// classic theme is forced statically via the html class in nuxt.config)
// ============================================================================
...
// GSAP — imported directly (the app used a Nuxt plugin; alias so the rest of
// this file's $gsap / $ScrollTrigger references keep working unchanged).
  ...
  if (plans.value && plans.value.length > 0) {
    return plans.value.map((p: any) => {
      ...
      return {
        ...
        annual:
          p.price_yearly ?? (p.price_monthly != null ? p.price_monthly * 10 : null),
...
const selectPlan = (slug?: string | null, contactOnly = false) => {
  // Contact-only tiers (Enterprise) and Signature onboarding route to a sales
  // conversation rather than self-serve signup.
  if (contactOnly || !slug) {
    navigateTo("mailto:hello@hoaconnect.info?subject=HOA%20Connect%20inquiry", {
      external: true,
    });
    return;
  }
  // Into the application (different origin) to start signup with the plan preselected.
  navigateTo(`${registerUrl}?plan=${slug}`, { external: true });
};
...
// Build a band label from unit bounds (e.g. 31–60 → "31–60 units").
  ...
  meta: [
    {
      name: "description",
      content:
        "Design-forward HOA software built by a Miami Beach creative agency. Florida-specific compliance tools, beautiful resident portals, and flat per-building pricing.",
    },
  ],
});
</script>
